...
class auth_recharge(Resource):
    ...
    def put(self):
        ...
        # 1. edit the balance of user
        value = ast.literal_eval(value)
        UserModel.recharge_balance(identity, value)

        # 2. create a transaction ***
        # transaction_id, type, amount_of_money, time, owner, trader
        transaction_id = str(uuid.uuid4())
        # check that the id is not used since uuid4 is generated by the random number
        while TransactionModel.query.filter_by(transaction_id = transaction_id).one_or_none() != None:
            transaction_id = str(uuid.uuid4())
        
        nowtime = datetime.datetime.now()
        transaction_time = nowtime.strftime("%Y-%m-%d %H:%M:%S")  # return type is string
        transaction = TransactionModel(transaction_id, "recharge", value, transaction_time, user.account, user.account) # here save user account
